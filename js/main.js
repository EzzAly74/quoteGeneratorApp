...
function generate() {
  var res = Math.floor(Math.random() * arr.length); //13
  testArray.push(res); //[13]
  ...
  document.getElementById("quote").innerHTML = arr[res].quote;
  var t = res;
  document.getElementById("author").innerHTML = "--" + arr[t].author;
  if (cont.classList.contains("test")) {
    cont.classList.remove("test");
    cont.classList.add("test2");
  } else {
    cont.classList.remove("test2");
